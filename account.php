...
?>
<html>
<head>  <title>OnWard-My Account</title>
...
<form method="post">
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-md-2">
                <input style="width:100%;font-size:20px;" class="btn btn-warning" type="submit" value="outdoor booking" name="outbook">
          </div>
          <div class="col-md-2">
...
<br><br>
<div class='pretty p-svg p-round p-plain p-jelly'>
   <input type='checkbox' name='color' value='hair colour'/>
   <div class='state p-primary'>
       <span class='svg' uk-icon='icon: check'></span>
       <label>hair color(60)</label>
   </div>
</div>
<br><br>

      nails<br/><select name='a1' size='4'>
       <option value='40'>beard(40)</option><br/>
       <option value='50'>moustache(50)</option><br/>
       <option value='70'>hair styling(70)</option><br/>
       <option value='80'>trimmimg(80)</option><br/>
      </select><br/><br/>
...
if(isset($_POST['outbook'])){
  ...
  <div class='pretty p-svg p-round p-plain p-jelly'>
     ...
     <div class='state p-primary'>
         <span class='svg' uk-icon='icon: check'></span>
         <label>facial(50)</label>
     </div>
  </div>
  <br><br>
  <div class='pretty p-svg p-round p-plain p-jelly'>
     <input type='checkbox'  name='styling' value='hair styling'/>
     <div class='state p-primary'>
         <span class='svg' uk-icon='icon: check'></span>
         <label>hair styling(100)</label>
     </div>
  </div>
  <br><br>
  <div class='pretty p-svg p-round p-plain p-jelly'>
  ...
  <div class='pretty p-svg p-round p-plain p-jelly'>
     ...
     <div class='state p-primary'>
         <span class='svg' uk-icon='icon: check'></span>
         <label>hair color(150)</label>
     </div>
  </div>
  <br><br>

    <input style='width:10%;font-size:20px;' class='btn btn-warning'type='submit' name='submit_out_order' value='place order'>
    </form>";
...
if(isset($_POST['orders'])){
  ...
  <input style='width:20%;font-size:20px;' class='btn btn-warning'type='submit' name='final_in_order' value='back to my account'>
    ...
    echo "invalid order id".mysqli_error($conn);
        ...
        if(isset($_POST['submit_out_order'])) {
          $count_out =0;
    ...
    if($count_out == 0){
      echo "<script>alert('please choose atleast one facility in oder to proceed further');</script>";
        ...
        echo "total payable amount = ".$count_out;
        echo "<form method='post'><input style='width:20%;font-size:20px;' class='btn btn-warning'
        type='submit' name='gotopayout' value='click for payment'></form>";
        $_SESSION['outdoorpayment']=$count_out;
        $_SESSION['outdoorvalues']=$total;
        }
}
...
if(isset($_POST['gotopayout'])){

  echo '<script type="text/javascript">';
echo 'window.location.href="paymentoutdoor.php";';
echo '</script>';
}
...
if(isset($_POST['edit'])){?>
  ...
  <body>
    <center>
    <form method="post" enctype="multipart/form-data" style="background-color:maroon;width:50%;">
        ...
        <input name="email" type="email">
          ...
          <p><p style="color:white;">age:</p>
            <input name="age" type="number">
              ...
              <p><p style="color:white;">number:</p>
                <input name="tel" type="tel">
                  </p>
                  <p><p style="color:white;">upload a picture:</p>
